...
Data: shot attempts by the [Sabrina Ionescu](https://en.wikipedia.org/wiki/Sabrina_Ionescu) in the 2023 WNBA season
  ...
  * Each row is a shot attempt by Sabrina Ionescu in the 2023 WNBA season
...
## Review: 1D kernel density estimation

::: columns
::: {.column}

* The KDE estimator is $$\displaystyle \hat{f}(x) = \frac{1}{n} \sum_{i=1}^n \frac{1}{h} K_h(x - x_i)$$
    
* Smooth out each observation into a small distribution (small "bump")

* Kernel functions are pdfs

* Put a copy of the kernel at each observation, then add them up

* This is a mixture

  * Basis distributions are the scaled copies of the kernel, centered at the $x_i$'s
  
  * Weight of each mixture component is $1/n$
